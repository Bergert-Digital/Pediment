...
class SeedNavTest extends WP_UnitTestCase {
	...
	public function test_menu_blocks_include_pediment_mega_menu() {
		$blocks = pediment_nav_menu_blocks();
		$this->assertStringContainsString( 'wp:pediment/mega-menu', $blocks );
		$this->assertStringContainsString( '"label":"Products"', $blocks );
		$this->assertSame( 1, substr_count( $blocks, 'wp:pediment/mega-menu' ) );
	}

	public function test_pristine_fallback_detection() {
		$this->assertTrue( pediment_nav_is_pristine_fallback( '<!-- wp:page-list /-->' ) );
		$this->assertTrue( pediment_nav_is_pristine_fallback( "\n <!-- wp:page-list /-->  \n" ) );
		$this->assertFalse( pediment_nav_is_pristine_fallback( pediment_nav_menu_blocks() ) );
		$this->assertFalse( pediment_nav_is_pristine_fallback( '' ) );
	}
	...
	public function test_creates_entity_when_none_exists() {
		...
		$this->assertSame( pediment_nav_menu_blocks(), $post->post_content );
		$this->assertSame( '1', get_post_meta( $id, PEDIMENT_NAV_MARKER, true ) );
		$this->assertSame( $id, pediment_nav_find_entity_id() );
	}
	...
	public function test_is_idempotent_and_preserves_user_edits() {
		...
		$again = pediment_nav_seed_entity();
	...
	public function test_adopts_pristine_page_list_fallback() {
		...
		$this->assertSame( pediment_nav_menu_blocks(), get_post( $id )->post_content );
		$this->assertSame( '1', get_post_meta( $id, PEDIMENT_NAV_MARKER, true ) );
	}
	...
	public function test_bind_ref_leaves_other_blocks_and_existing_ref_alone() {
		...
		$paragraph = pediment_nav_bind_ref(
			array(
