 ...
 * @return bool
 */
function starter_nav_path_is_current( string $link_url, string $current_path ): bool {
 ...
 * @return string
 */
function starter_nav_mark_active_link( string $block_content, array $block ): string {
	...
	if ( ! starter_nav_path_is_current( (string) $block['attrs']['url'], $current_path ) ) {
		return $block_content;
	}

	return preg_replace( '/<a\b/', '<a aria-current="page"', $block_content, 1 );
}
add_filter( 'render_block_core/navigation-link', 'starter_nav_mark_active_link', 10, 2 );
